...
const ProductCard = ({ product }: ProductCardProps) => {
  return (
    <div>
      <div className="relative h-72 flex items-center justify-center cursor-pointer group p-8">
        <img
          src={product.image}
          alt={product.title}
          className=" h-full object-contain transition hover:scale-105"
        />
        <div className="absolute top-0 left-0 w-full h-0 flex flex-col justify-center items-center bg-opacity-20 bg-black opacity-0 group-hover:h-full group-hover:opacity-100 duration-300">
          <Link
